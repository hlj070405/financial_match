...
def _is_stale_key(key: str, stale_type: int) -> bool:
    """读取已存在的缓存数据并判断是否过时（不删除，用于 stale-while-revalidate）"""
    c = _get_client()
    if c is None:
        return False
    try:
        ...
        if raw is None:
            return False
        data = json.loads(raw)
        return _is_stale(data, stale_type)
    except Exception:
        return False
...
def cache_get(key: str, stale_type: int = 0) -> Optional[List[Dict]]:
    ...
    - 数据过时（按 stale_type）→ 返回旧数据（调用方负责异步刷新）
    - 其余情况 → 返回数据
    """
    c = _get_client()
    if c is None:
    ...
    try:
        ...
        if not _is_valid(data):
            ...
            return None
        # 过时但有效：返回旧数据，由调用方触发异步刷新（stale-while-revalidate）
        return data
    except Exception:
        return None
