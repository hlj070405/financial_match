<template>
  <div class="h-full flex flex-col gap-6 p-2 font-sans overflow-y-auto custom-scrollbar">
    ...
    <!-- Loading State -->
    <div v-if="loading && !diagnosisData" class="flex-1 flex flex-col items-center justify-center">
    ...
    <template v-else>
      <!-- Demo Banner -->
      <div v-if="isDemo" class="bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 flex items-center justify-between shrink-0">
        <p class="text-[11px] text-blue-600"><span class="font-bold">示例数据</span> — 当前展示的是AI分析示例，输入公司名称可获取实时分析</p>
        <button @click="clearDemo" class="text-[10px] text-blue-500 hover:text-blue-700 font-medium">清除示例</button>
      </div>
      <!-- Summary Banner -->
...
const clearDemo = () => { diagnosisData.value = null; isDemo.value = false }
