  ...
  <div class="h-full w-full">
    <div v-if="showTabs" class="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center justify-between shrink-0">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-bold text-gray-900">产业链分析</span>
      </div>
...
import ChainMapPage from './pages/ChainMapPage.vue'
import SupplyRiskPage from './pages/SupplyRiskPage.vue'
import CompetePage from './pages/CompetePage.vue'

const props = defineProps({ activeFeature: { type: String, default: '' } })
const moduleDef = ALL_MODULES.find(m => m.id === 'chain')
const features = moduleDef?.features || []
const featureMap = { 'chain_map': ChainMapPage, 'chain_risk': SupplyRiskPage, 'chain_compete': CompetePage }
const currentFeature = ref(props.activeFeature || 'chain_map')
const transitionName = ref('page-slide-right')
let prevIndex = 0
const showTabs = computed(() => features.length > 1)
const currentComponent = computed(() => featureMap[currentFeature.value] || ChainMapPage)
const switchTo = (id) => {
...
watch(() => props.activeFeature, (val) => { if (val && val.startsWith('chain_') && val !== currentFeature.value) switchTo(val) })
</script>
