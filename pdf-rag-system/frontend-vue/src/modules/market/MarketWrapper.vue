    ...
    <div v-if="showTabs" class="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center justify-between shrink-0">
      <div class="flex items-center gap-2">
        <div class="p-2 rounded-xl bg-amber-50">
          <CandlestickChart class="w-4 h-4 text-amber-600" />
        </div>
        <span class="text-sm font-bold text-gray-900">行情终端</span>
      </div>
...
const moduleDef = ALL_MODULES.find(m => m.id === 'market')
...
watch(() => props.activeFeature, (val) => {
  if (val && val.startsWith('mkt_') && val !== currentFeature.value) switchTo(val)
})
</script>
