  ...
  <div class="h-full w-full">
    <div v-if="showTabs" class="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center justify-between shrink-0">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-bold text-gray-900">智能检索</span>
      </div>
...
const moduleDef = ALL_MODULES.find(m => m.id === 'rag')
const features = moduleDef?.features || []
const featureMap = { 'rag_search': SearchPage, 'rag_rank': RankPage, 'rag_filter': FilterPage }
...
watch(() => props.activeFeature, (val) => { if (val && val.startsWith('rag_') && val !== currentFeature.value) switchTo(val) })
</script>
