  ...
  <div class="h-full w-full">
    <div v-if="showTabs" class="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center justify-between shrink-0">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-bold text-gray-900">一键研报</span>
      </div>
...
import DeepReportPage from './pages/DeepReportPage.vue'
import TemplatePage from './pages/TemplatePage.vue'

const props = defineProps({ activeFeature: { type: String, default: '' } })
const moduleDef = ALL_MODULES.find(m => m.id === 'report')
const features = moduleDef?.features || []
const featureMap = { 'rpt_summary': SummaryPage, 'rpt_deep': DeepReportPage, 'rpt_template': TemplatePage }
const currentFeature = ref(props.activeFeature || 'rpt_summary')
const transitionName = ref('page-slide-right')
let prevIndex = 0
const showTabs = computed(() => features.length > 1)
const currentComponent = computed(() => featureMap[currentFeature.value] || SummaryPage)
const switchTo = (id) => {
...
watch(() => props.activeFeature, (val) => { if (val && val.startsWith('rpt_') && val !== currentFeature.value) switchTo(val) })
</script>
